...
export default class Spring {
  ...
  Step(deltaTime) {
    ...
    if (dampingRatio === 1) {
      ...
      if (c > EPS) {
        ...
        const cSquared = c ** 2;
        z = frequencyStep + (frequencyStep ** 2 * cSquared * cSquared / 20 - cSquared) * frequencyStep ** 3 / 6;
      }
      /*
      ...
      if (cFrequency > EPS) {
        ...
        y = deltaTime + (deltaTime ** 2 * cFrequencySquared * cFrequencySquared / 20 - cFrequencySquared) * deltaTime ** 3 / 6;
      }
      const goalDistance = position - goal;
  ...
  CanSleep() {
    return this.Velocity ** 2 > SLEEP_VELOCITY_SQ_LIMIT || (this.Goal - this.Position) ** 2 > SLEEP_OFFSET_SQ_LIMIT ? false : true;
  }
  GetGoal() {
    return this.Goal;
  }
